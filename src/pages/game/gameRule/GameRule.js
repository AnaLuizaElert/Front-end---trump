...
function GameRule() {
  ...
  const [chosenAttribute, setChosenAttribute] = useState({ "attribute": "", "numRound": 1 });
  ...
  const chooseAtribute = (attribute) => {
    setChosenAttribute({ "attribute": attribute, "numRound": chosenAttribute.numRound + 1 });
  ...
  function chooseAtributeComputer() {
    ...
    setChosenAttribute({ "attribute": attributes[0], "numRound": chosenAttribute.numRound + 1 });
  };
  ...
  async function updateUser() {
    try {
      const user = await UserService.showOneByName(localStorage.getItem("user"));
      let editedUser;
  ...
  return (
    <div className='simple-background-game' id='game'>
      ...
      <div className='battle'>
        <button className='buttonGiveUp buttonGame' onClick={() => giveUp()}>Give up</button>
      </div>
